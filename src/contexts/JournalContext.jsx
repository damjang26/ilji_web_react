    ...
    const createJournalEntry = useCallback(
        async (journalPayload) => {
            try {
                ...
                // 서버에서 보낸 구체적인 에러 메시지가 있다면 그것을 사용합니다.
                if (err.response && err.response.data && err.response.data.message) {
                    // 예: "이미지 파일의 용량은 10MB를 초과할 수 없습니다."
                    alert(`Failed to save: ${err.response.data.message}`);
                } else {
                    // 그 외 네트워크 오류 등 예상치 못한 문제 발생 시
                    alert("An unknown error occurred while saving the journal. Please try again later.");
                }
                // 에러를 다시 던져서, 호출한 컴포넌트(예: JournalForm)에서 추가적인 처리(로딩 상태 해제 등)를 할 수 있게 합니다.
                throw err;
            }
        },
        [user]
    ); // `user`가 변경될 때 함수가 최신 `user` 정보를 참조하도록 의존성 배열에 추가합니다.
        ...
        async (logId, journalPayload) => {
            try {
                ...
                if (err.response && err.response.data && err.response.data.message) {
                    ...
                    alert("An unknown error occurred while updating the journal. Please try again later.");
                }
                throw err;
            }
        },
